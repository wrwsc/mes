...
def create_access_token(data: dict, expires_delta: timedelta = None) -> str:
    ...
    # Если указан параметр expires_delta, то к текущему времени добавляется указанный интервал времени для определения времени истечения токена. В противном случае устанавливается срок действия в 1 час от текущего времени.
    if expires_delta:
        expire = datetime.now(timezone.utc) + expires_delta
    else:
        expire = datetime.now(timezone.utc) + timedelta(hours=1)
    to_encode.update({"exp": expire}) # обновление данных в переменной to_encode посредством добавления информации о времени истечения токена
    auth_data = get_auth_data() # получение данных для аутентификации
    encode_jwt = jwt.encode(to_encode, auth_data['secret_key'], algorithm=auth_data['algorithm']) # использование библиотеки JWT для кодирования данных в переменной to_encode с использованием секретного ключа и алгоритма шифрования из данных аутентификации
    return encode_jwt
